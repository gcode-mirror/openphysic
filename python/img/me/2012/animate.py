...
Nbsize=100 # nb de tranches dans une image
for i in range(0, Nb):
	outfile_region="temp/temp_%.4d.png" % i
	if i<=Nb-Nbsize:
		i1=i
		i2=i+Nbsize-1
		print "%s : %d-%d" % (outfile_region, i1, i2)
		region=im.crop((i1*W/Nb, 0, (i2+1)*W/Nb, H))
		region.save(outfile_region, "PNG")
	else:
		i1=i
		i2=Nb-1
		i3=0
		...
		x1, y1 = part1.size
		x2, y2 = part2.size
		region = Image.new("RGB", (x1+x2,H), "white")
